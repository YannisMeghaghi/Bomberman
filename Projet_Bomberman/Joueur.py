...
class Bombe :
    def __init__(self, canvas, personnage):
                ...
                self.id = canvas.create_image(30, 30, image=self.photo)
                self.compteur = Timer(1, self.explode)
##                while compteur != 1:
##                        time.sleep(3.0)
##                        compteur =+ 1
    ...
    def explode(self): # Pour effacer le canvas de la bombe
        ...
        if self.touche_un_joueur1() and self.touche_un_joueur2():
                #JEU = False
                    ...
                    #bouton=Button(Fenêtre, text="Recommencer", command=Fenêtre.destroy).pack()
                   # bouton2=Button(Fenêtre,text="Quitter", command= Fenêtre.destroy).pack()
                   
##                    Canvas(fenetre, width=250, height=100, bg='ivory').pack(side=TOP, padx=5, pady=5)
##                      bouton=Button(fenetre, text ='Recommencer').pack(side=LEFT, padx=5, pady=5)
##                    bouton2=Button(fenetre, text ='Quitter', command = fenetre.destroy).pack(side=RIGHT, padx=5, pady=5)
##                    resultat = askretrycancel('Fin de jeu', 'Malheuresement vous avez été tous les deux mauvais : \n Egalite')
##                   
##                    canvas22 = Canvas2(fenetre, width=250, height=100, bg='ivory').pack()(side=TOP, padx=5, pady=5)
##                    bouton=Button(fenetre, text ='Recommencer', command= Recommencer).pack()(side=LEFT, padx=5, pady=5)
##                    bouton2=Button(fenetre, text ='Quitter', command = fenetre.destroy).pack()(side=RIGHT, padx=5, pady=5)
##                    if bouton is True:
##                        Fenêtre.destroy
##                    elif bouton2 is True :
##                        print('ok')
                       # bouton2= Button (fenetre, text= "Annuler", command = fenetre.quit).pack(side = LEFT, padx = 5, pady = 5)        
                        #bouton3=Button(fenetre,text="Quitter", command= Fenêtre.quit).pack(side = RIGHT, padx = 5, pady = 5)
                       
##                    label = Label (fenetre, text= 'Malheuresement vous avez été tous les deux mauvais : \n Egalite')
                       
                fin_de_partie("Egalité ! C'est rare... Auriez vous fait exprès?")
                #print('égalité')
        elif self.touche_un_joueur2() and not(self.touche_un_joueur1()):
                #JEU = False
                fin_de_partie("Le Joueur 2 est mort, vraiment pathétique...")
                #print("Victoire Joueur 1")
        elif self.touche_un_joueur1()and not(self.touche_un_joueur2()):
                #JEU = False
                fin_de_partie("Le Joueur 1 a passé l'arme à gauche, repose en paix...")
    ...
    def touche_un_joueur1(self):
                ...
                if y_bombe == y_joueur and x_joueur >= x_bombe - 1 and x_joueur <= x_bombe +1 or x_bombe == x_joueur and y_joueur >= y_bombe - 1 and y_joueur <= y_bombe +1:
                        ...
                        print("condition_oui1","Coordonnées de la bombe posé:",x_bombe, y_bombe,
                              "\n","Coordonnées du Joueur1:",Pos_joueur1,
                              "\n","Coordonnées du Joueur2:",Pos_joueur2)
                        return 1
                else:
                        print("condition_non1","Coordonnées de la bombe posé:" ,x_bombe, y_bombe,
                              "\n","Coordonnées du Joueur1:",Pos_joueur1,
                              "\n", "Coordonnées du Joueur2:",Pos_joueur2)
                        return 0
    ...
    def touche_un_joueur2(self):
                ...
                if y_bombe == y_joueur2 and x_joueur2 >= x_bombe - 1 and x_joueur2 <= x_bombe +1 or x_bombe == x_joueur2 and y_joueur2 >= y_bombe - 1 and y_joueur2 <= y_bombe +1:
                        ...
                        print("condition_oui2","Coordonnées de la bombe posé:",x_bombe, y_bombe,
                              "\n","Coordonnées du Joueur1:",Pos_joueur1,
                              "\n","Coordonnées du Joueur2:",Pos_joueur2)
                        return 1
                else:
                        print("condition_non2","Coordonnées de la bombe posé:" ,x_bombe, y_bombe,
                              "\n","Coordonnées du Joueur1:",Pos_joueur1,
                              "\n", "Coordonnées du Joueur2:",Pos_joueur2)
                        return 0
...
def fin_de_partie(c):
    global fenetre
    fenetre = Tk()
    canvas_fin = Canvas(fenetre,width = 300, height = 200)
    canvas_fin.create_text(150,100, text =c)
    bouton=Button(fenetre, text ='Recommencer', command = Recommencer).pack(side=RIGHT, padx=5, pady=5)
    bouton2=Button(fenetre, text ='Quitter', command = destroy).pack(side=RIGHT, padx=5, pady=5)
    canvas_fin.pack()
    canvas_fin.mainloop()
    fenetre.mainloop()
...
def Recommencer():
    ...
    if Bombe.explode :
        Player.case = [0,0]
        Player2.case=[9,9]
        Player.canvas.coords(Player.id, 30, 30)
        Player2.canvas.coords(Player2.id, 570, 570)
        print('Recommencer')
...
if __name__=="__main__":
    ...
    grille = Decor.grille(Zone, 2, 'grey')
    Decor.dessine_incassables(Zone)
    Player = Personnage(Zone, 'red')
    Player2 = Personnage2(Zone, 'blue')
    #Recommencer = Recommencer (Personnage, Personnage2)
    #grille(2, "black")
